...
void maxabsElemArrayOnCPU(float *data, int const size) {
    ...
    for (int i = 1; i < size-1; i++) {
        data[0] = data[i] > data[0] ? data[i] : data[0] ;
    }
}
...
//     ERR_SAFE(cudaDeviceSynchronize());
//     iStart = time_of_day_seconds();
//     Max_Interleaved_Addressing_Global<<<grid, block>>>(d_idata, size);
//     ERR_SAFE(cudaDeviceSynchronize());
//     iElaps = time_of_day_seconds() - iStart;

    float gpu_max;
    ERR_SAFE(cudaMemcpy(&gpu_max, d_idata, sizeof(float), cudaMemcpyDeviceToHost));
    printf("gpu reduce      elapsed %f sec gpu_max: %f\n", iElaps, gpu_max);

    ERR_SAFE(cudaFree(d_idata));

    printf("Results Match? %s\n", (gpu_max==cpu_max)? "true" : "false");
